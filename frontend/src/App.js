...
const USER_ID = (typeof window !== 'undefined' && (localStorage.getItem('auth_user_id') || localStorage.getItem('auth_email') || (new URLSearchParams(window.location.search)).get('email'))) || 'demo-user-123';
...
const ChatInterface = ({ device, messages, onSendMessage, deviceNotifications, onMarkNotificationRead, reloadChat, loadFullHistory, onDeviceUpdated }) => {
  ...
  const handleKeyPress = async (e) => {
    // On any typing, expand to full history for better context visibility
        ...
        <div className="p-4">
          <div className="flex space-x-2">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg ${
                showMediaInput 
                  ...
                  : 'text-faint hover:text-soft hover:ring-1 hover:ring-sky-600/50'
              }`}
            ...
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 border border-blue-soft bg-[#0D1320] text-soft"
            />
            <button
              onClick={handleSend}
              disabled={!inputMessage.trim() && selectedFiles.length === 0 && mediaUrls.filter(url => url.trim()).length === 0 && selectedNotifications.length === 0}
              className="px-4 py-2 rounded-lg btn-primary hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Send size={18} />
            </button>
          </div>
...
const NotificationsList = ({ notifications, devices, onMarkRead, onNavigateToDevice }) => {
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (device && onNavigateToDevice) {
      ...
      console.error('Cannot navigate - device not found or callback missing', { 
        device_id: notification.device_id, 
        device_found: !!device,
        callback_exists: !!onNavigateToDevice 
      });
    }
  };
